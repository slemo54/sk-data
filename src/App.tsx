    ...
    <div className="app-shell min-h-screen bg-background">
      ...
      <header className="app-header">
        <div className="header-left">
          ...
          <p className="text-sm text-muted-foreground">Dashboard Persone Unificata: Wine Awards + GuildSomm</p>
        </div>
        <div className="header-right">
          <Select defaultValue="vinitaly2024">
            <SelectTrigger className="w-[200px]">
              <SelectValue placeholder="Seleziona evento" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="vinitaly2024">Vinitaly 2024 Abroad</SelectItem>
            </SelectContent>
          </Select>
        ...
        <section className="sub-filters">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                checked={Boolean(filters.approved)}
            ...
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                checked={Boolean(filters.contacted)}
                onChange={(e) => handleFilterChange({ contacted: e.target.checked ? true : undefined })}
              />
              Contattati
            </label>
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                ...
                onChange={(e) => handleFilterChange({ assignedToMe: e.target.checked, userId })}
              />
              Assegnati a me
            </label>
            <label className="flex items-center gap-2 text-sm cursor-pointer">
            ...
            <label className="flex items-center gap-2 text-sm">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setUserId(val);
                  localStorage.setItem('skdb_user_id', val);
                }}
                placeholder="ID"
                className="w-28 h-8"
              />
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead onClick={() => handleSort('full_name')} className="cursor-pointer whitespace-nowrap">
                      Social Accounts
                    </TableHead>
                  ...
                  {!loading &&
                    contacts.map((contact) => (
                      ...
                      >
                            ...
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                              {contact.instagram_url && (
                                <span className="flex items-center gap-1">
                                  ...
                                  @{getSocialHandle(contact.instagram_url)}
                                </span>
                              )}
                              {contact.linkedin_url && (
                                <span className="flex items-center gap-1">
                                  <Linkedin className="h-3 w-3 text-blue-700" />
                                  {getSocialHandle(contact.linkedin_url)}
                                </span>
                              )}
                              {!contact.instagram_url && !contact.linkedin_url && (
                                <span className="text-muted-foreground/60">Nessun social</span>
                              )}
                            </div>
                          </div>
                        </TableCell>
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <button
                            onClick={() => void handleToggleContacted(contact)}
                            className="focus:outline-none"
                          >
                            {contact.contacted ? (
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center gap-2">
                            <Popover
                              ...
                              onOpenChange={(open) => {
                                setNoteOpenId(open ? contact.id : '');
                                if (open) setNoteDraft(contact.notes ?? '');
                              }}
                            >
                              ...
                              <PopoverContent className="w-80" align="end">
                                <div className="flex flex-col gap-2">
                                  ...
                                  <div className="flex justify-end gap-2">
                                    <Button size="sm" variant="ghost" onClick={() => setNoteOpenId('')}>
                                      Annulla
                                    </Button>
                                    <Button size="sm" onClick={() => void handleSaveNote(contact.id)}>
                                      Salva
                                    </Button>
                                  </div>
                                </div>
            ...
            <div className="mt-6 flex flex-col gap-6">
              <div className="detail-grid">
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <Input
                    ...
                    onChange={(e) => setDraftValue('email', e.target.value)}
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <Input
                    ...
                    onChange={(e) => setDraftValue('instagram_url', e.target.value)}
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <Input
                    ...
                    onChange={(e) => setDraftValue('linkedin_url', e.target.value)}
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <Input
                    ...
                    onChange={(e) => setDraftValue('employer', e.target.value)}
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm">
                ...
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <Input
                    ...
                    onChange={(e) => setDraftValue('occupation', e.target.value)}
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm">
                  <span className="font-medium">Next Action</span>
                  <Select
                    value={detailDraft.next_action ?? 'none'}
                    onValueChange={(v) =>
                      setDetailDraft((prev) => ({
                        ...prev,
                        next_action: v === 'none' ? null : (v as NextAction),
                      }))
                    }
                  >
                    <SelectTrigger>
                ...
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setDraftValue('notes', e.target.value)}
                  />
                </label>
              </div>

              <div className="flex items-center gap-3">
                <Button onClick={() => void handleSaveDetail()} disabled={saving}>
                  {saving ? 'Salvataggio...' : 'Salva'}
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => void handleStatusAdvance(selectedContact.id, selectedContact.status)}
                >
                  Avanza stato ({selectedContact.status})
                </Button>
              </div>
              ...
              <div>
                ...
                {!selectedSources.length && (
                  <p className="text-sm text-muted-foreground">Nessuna provenance disponibile.</p>
                )}
                {!!selectedSources.length && (
